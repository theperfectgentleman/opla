...
Grounded in code as of 2026-09-06 (`main` @ `e5d8bc8`):
...
- **Players:** Expo agent/pulse/yard routes; SDUI `FormRenderer` + field widgets; AsyncStorage offline queue for submissions and attendance (not WatermelonDB). Public web form at `/s/:slug` (`PublicForm.tsx`).
...
4. Close remaining Design gaps only if a real FMCG/interview form is blocked.
