...
class ProjectCatalogService:
    @staticmethod
    def list_items(db: Session, project_id: uuid.UUID) -> list[ProjectCatalogItem]:
        return (
            db.query(ProjectCatalogItem)
            .filter(ProjectCatalogItem.project_id == project_id)
            .order_by(ProjectCatalogItem.is_active.desc(), ProjectCatalogItem.label.asc())
            .all()
        )

    @staticmethod
    def get_item_or_404(db: Session, project_id: uuid.UUID, item_id: uuid.UUID) -> ProjectCatalogItem:
        item = (
            db.query(ProjectCatalogItem)
            .filter(ProjectCatalogItem.project_id == project_id, ProjectCatalogItem.id == item_id)
            .first()
        )
        if not item:
    ...
    @staticmethod
    def delete_item(db: Session, project: Project, item: ProjectCatalogItem) -> None:
        ProjectAccessService.ensure_project_is_mutable(project)
        db.delete(item)
        db.commit()