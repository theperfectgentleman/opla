...
import { formAPI, agentFormAPI } from '../../../../services/api';
...
export default function FormStartScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const linked = await agentFormAPI.getLinkedForms(id);
  ...
  const handlePreloadOffline = async () => {
    ...
    try {
      ...
      const context = { mode: 'agent' as const, formId: id, slug: meta.slug };
  ...
  const handleOpenDeviceRecords = () => {
    router.push({
      pathname: '/(main)/(agent)/device-records/[id]',
      params: {
        id,
        orgId,
        projectId,
