...
import { agentFormAPI } from '../../../../services/api';
...
export default function DeskFormScreen() {
  ...
  const silentFlush = useCallback(async () => {
    try {
      await flush(agentFormAPI.submit);
  ...
  const handleSyncNow = async () => {
    ...
    try {
      await agentFormAPI.submit(formId, data, metadata);
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#0f172a', justifyContent: 'center', alignItems: 'center', padding: 32 }}>
        ...
        <TouchableOpacity
          onPress={() => {
            router.replace({
              pathname: '/(main)/(agent)/form/[id]' as any,
        ...
        <TouchableOpacity
          onPress={() => router.push({
            pathname: '/(main)/(agent)/device-records/[id]' as any,
            params: { id, orgId, projectId, projectName, formTitle: form?.meta?.title ?? formTitle, orgColor: accent },
          })}
          style={{
