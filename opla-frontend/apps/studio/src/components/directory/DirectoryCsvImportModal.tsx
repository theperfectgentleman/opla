...
import { slugifyDirectoryKey } from '../../utils/directoryUtils';
import {
    autoMatchDirectoryColumns,
    buildDirectoryImportRows,
    decodeMappingValue,
    ...
    parseDirectoryPaste,
    validateDirectoryImportRow,
    type DirectoryColumnMapping,
    type DirectoryFieldGeneratorType,
    type DirectoryImportRow,
    type DirectoryOrphanGenerators,
} from '../../utils/directoryCsvImport';

export type DirectoryCsvImportModalProps = {
    open: boolean;
    onClose: () => void;
...
function getFieldLabel(field: DirectoryField | undefined, bind: string, keyFieldId?: string | null, labelFieldId?: string | null) {
...
function getMappingHelperText(
    value: string,
    fieldByBind: Map<string, DirectoryField>,
    keyFieldId?: string | null,
    labelFieldId?: string | null,
): string | null {
...
const MappingSelect: React.FC<MappingSelectProps> = ({
    ...
    return (
        ...
        >
            ...
            <optgroup label="Directory fields">
                {fields.map((field) => {
...
const DirectoryCsvImportModal: React.FC<DirectoryCsvImportModalProps> = ({
    open,
    onClose,
    fields,
    keyFieldId,
    ...
    const [columnMapping, setColumnMapping] = useState<DirectoryColumnMapping>({});
    const [orphanGenerators, setOrphanGenerators] = useState<DirectoryOrphanGenerators>({});
    const [importRows, setImportRows] = useState<DirectoryImportRow[]>([]);
    ...
    const fieldByBind = useMemo(() => {
        const map = new Map<string, DirectoryField>();
    ...
    const refreshImportRows = useCallback((
        ...
        orphans: DirectoryOrphanGenerators,
        preserveStatuses?: Map<string, DirectoryImportRow['status']>,
    ) => {
        const built = buildDirectoryImportRows(rows, mapping, fields, keyFieldId, labelFieldId, orphans);
        setImportRows((prev) => {
    ...
    const handleParse = () => {
        const result = parseDirectoryPaste(pasteText, fields, keyFieldId, labelFieldId);
        if (result.error) {
    ...
    const clearOrphanForBind = (bind: string, orphans: DirectoryOrphanGenerators) => {
        if (!orphans[bind]) {
            return orphans;
        }
        const next = { ...orphans };
    ...
    const handleOrphanGeneratorChange = (bind: string, generator: DirectoryFieldGeneratorType | '') => {
        let nextOrphans = { ...orphanGenerators };
        if (!generator) {
    ...
    const handleRematch = () => {
        const nextMapping = autoMatchDirectoryColumns(csvHeaders, fields, keyFieldId, labelFieldId);
        let nextOrphans: DirectoryOrphanGenerators = {};
        ...
        setImportRows((prev) =>
            prev.map((row) => {
                ...
                if (
                    ...
                    nextData = { ...nextData, [keyFieldId]: slugifyDirectoryKey(value) };
                }

                const errors = validateDirectoryImportRow(nextData, fields, keyFieldId, labelFieldId);
                return {
                    ...row,
                    data: nextData,
                    errors,
    ...
    const saveImportRow = async (rowId: string) => {
        ...
        try {
            ...
            const message = error?.response?.data?.detail || error?.message || 'Could not save directory entry.';
            setImportRows((prev) =>
                            ...
                            {unmappedFields.length > 0 && (
                                <section className="space-y-2">
                                    ...
                                    <p className="text-[10px] text-[hsl(var(--text-tertiary))]">
                                        These directory fields are not mapped from any CSV column. Choose a generator or leave empty.
                                    </p>
                                    <div className="overflow-x-auto rounded-xl border border-[hsl(var(--border))]/50">
                                        <table className="w-full border-collapse text-xs min-w-[480px]">
                                            <thead className="bg-[hsl(var(--surface-elevated))]/80">
                                                <tr className="border-b border-[hsl(var(--border))]/50 text-[10px] font-bold uppercase tracking-wider text-[hsl(var(--text-secondary))]">
                                                    <th className="p-2 text-left">Directory field</th>
                                                    ...
                                                    return (
                                                        <tr key={bind}>
                                                            ...
                                                            <td className="p-2">
                                                                <select
                                                                    value={currentOrphan}
                                                                    onChange={(e) => handleOrphanGeneratorChange(bind, e.target.value as DirectoryFieldGeneratorType | '')}
                                                                    className="w-full rounded-md border border-[hsl(var(--border))]/60 bg-[hsl(var(--background))] px-2 py-1 text-xs outline-none focus:border-[hsl(var(--primary))]/50"
                                                                >
...
export default DirectoryCsvImportModal;
