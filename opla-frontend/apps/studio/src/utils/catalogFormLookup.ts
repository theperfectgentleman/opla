...
export async function hydrateBlueprintCatalogForms(
    ...
    consumerFormId: string,
    client: CatalogFormLookupClient,
    ...
    await Promise.all(
        catalogFormIds.map(async (catalogFormId) => {
            try {
                const response = await client.getOptions(consumerFormId, catalogFormId);
...
export function catalogFormEntriesFromOptions(options: CatalogLookupOption[]) {
