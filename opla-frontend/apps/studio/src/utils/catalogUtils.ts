...
export type CatalogBlueprintSource = {
    status?: string;
...
const CATALOG_ENTRY_EXCLUDED_TYPES = new Set([
    'gps_capture',
    'photo_capture',
    'file_upload',
    'signature_pad',
...
export function extractBlueprintFields(blueprint?: CatalogBlueprintSource['blueprint_live']) {
    const uiFields: CatalogField[] = [];
    blueprint?.ui?.forEach((screen) => {
        screen.children?.forEach((field) => {
            const fieldType = String(field.type || '');
            if (CATALOG_ENTRY_EXCLUDED_TYPES.has(fieldType)) {
                return;
            }
            uiFields.push(field as CatalogField);
        });
    });
    return uiFields;
}

export function getCatalogBlueprintFields(formDetail: CatalogBlueprintSource) {
...
export function countUnpublishedCatalogFields(formDetail: CatalogBlueprintSource) {
...
export function normalizeCatalogEntry(entry: Record<string, any>) {
    return {
        ...entry,
...
export function slugifyCatalogKey(label: string | null | undefined): string {
    return String(label ?? '')
        .trim()
        .toLowerCase()
        .replace(/\s+/g, '_')
...
export function getCatalogFieldDisplayValue(field: CatalogField, rawValue: unknown): string {
...
export function createEmptyRowData(fields: CatalogField[]): Record<string, string> {
...
export function entryToRowData(entry: Record<string, any>, fields: CatalogField[]): Record<string, string> {
