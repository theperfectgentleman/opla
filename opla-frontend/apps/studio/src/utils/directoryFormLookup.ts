...
export type DirectoryFormLookupClient = {
    listSources: (consumerFormId: string) => Promise<Array<{ id: string; title: string }>>;
    getOptions: (
        consumerFormId: string,
        directoryFormId: string,
    ) => Promise<{ options: DirectoryLookupOption[]; value_field?: string; label_field?: string }>;
};
...
export async function hydrateBlueprintDirectoryForms(
    ...
    consumerFormId: string,
    client: DirectoryFormLookupClient,
    ...
    await Promise.all(
        directoryFormIds.map(async (directoryFormId) => {
            try {
                const response = await client.getOptions(consumerFormId, directoryFormId);
...
export function directoryFormEntriesFromOptions(options: DirectoryLookupOption[]) {
    return buildSourceItems(options);
}
